...
// Base URL for geo API. Default to local backend search endpoint host:port
const GEO_API = process.env.NEXT_PUBLIC_GEO_API ?? "http://localhost:5000/api/geo";
...
// Predefined YOE options for specific states to avoid waiting on fetched data
const PREDEFINED_YOES: Record<string, string[]> = {
  "Andhra Pradesh": ["2019", "2024", "2029"],
  Bihar: ["2025"],
  "Tamilnadu": ["2021", "2026"],
};
    ...
    features: data
      ...
      .map((d) => ({
        ...
        properties: {
          ...
          value: d.value,
          district: d.district,
          // If a forced color is provided (e.g. when the user filtered by a
          // specific `value`), use it for all points; otherwise use the
          // mapped color for the point's value.
          color: forcedColor ?? valueToColor.get(d.value) ?? VALUE_COLORS.default,
        },
      })),
  };
}
...
export default function GeoTest() {
  ...
  const [filterOptions, setFilterOptions] = useState<{
    ...
    yoeValues: string[];
    valueOptions: string[];
  }>({ states: Object.keys(STATE_BOUNDS).sort(), yoeValues: [], valueOptions: [] });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Use backend /search route which accepts any combination of params
      const base = "http://localhost:5000/api/geo";
      const url = params.toString() ? `${base}/search?${params}` : `${base}/search`;
  ...
  useEffect(() => {
    ...
    const displayedData = hasFilter
      ? data.filter((d) =>
          (stateFilter ? d.state === stateFilter : true) &&
    ...
    const forcedColor = valueFilter
      ? VALUE_COLORS[valueFilter] ?? getColorForValue(valueFilter, 0)
      : undefined;
    const geoJson = buildGeoJson(displayedData, valueToColor, forcedColor);
    ...
    const addOrUpdateLayers = () => {
      ...
      // If there are no features to display, leave the map empty (no source/layers)
      if (!geoJson.features || geoJson.features.length === 0) {
        return;
      }

      // Add source without clustering so every point is displayed individually
      ...
      // Show each point as a small circle (no cluster aggregation)
      m.addLayer({
        id: "geo-points",
        type: "circle",
        source: "geo-points",
  ...
  // If we have predefined YOEs for the selected state, prefer those (faster UX).
  // Otherwise fall back to the YOE values derived from fetched rows.
  const displayedYoeOptions = stateFilter && PREDEFINED_YOES[stateFilter]
    ? PREDEFINED_YOES[stateFilter]
    : yoeValues;
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setStateFilter(s);
                  if (s && PREDEFINED_YOES[s] && PREDEFINED_YOES[s].length > 0) {
                    setYoeFilter(PREDEFINED_YOES[s][0]);
                  } else {
                    setYoeFilter("");
                  }
                }}
